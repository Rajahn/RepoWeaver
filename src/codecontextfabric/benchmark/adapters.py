"""Benchmark adapters — a common interface so Code Context Fabric can be measured
side-by-side with other tools without ever vendoring their code.

- `CodeContextFabricAdapter` — the real, fully-implemented adapter for this project.
...
from codecontextfabric.benchmark.metrics import BenchmarkMetrics, collect_metrics
...
@dataclass
class CodeContextFabricAdapter:
    """The real adapter: builds and measures Code Context Fabric's own index."""
...
class ExternalCommandAdapter:
    """Generic adapter for any external tool that speaks Code Context Fabric's simple
...
class _OptionalCLIAdapter:
    ...
    installed. Never executes proprietary logic on Code Context Fabric's behalf —
    ...
    def run(
        ...
        if shutil.which(self.cli_name) is None:
            raise AdapterSkipped(
                ...
                "(install it yourself to benchmark it; Code Context Fabric does not "
...
class GrepBaselineAdapter:
    ...
    def run(
        ...
        # Reuse Code Context Fabric's own symbol list purely to pick a fair, repo-agnostic
...
def build_adapter(
    ...
    CodeContextFabricAdapter
    | ExternalCommandAdapter
    | CodeGraphAdapter
    | GraftAdapter
    | GrepBaselineAdapter
):
    """`spec` is an adapter name ("ccf", "grep", "codegraph", "graft")
