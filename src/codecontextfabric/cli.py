"""Code Context Fabric CLI — `ccf` command entry point (`fabric` is a compat alias)."""
...
from codecontextfabric import __version__
from codecontextfabric.benchmark.cli import app as benchmark_app
from codecontextfabric.cli_errors import exit_on_locked_db
...
from codecontextfabric.typed.cli import app as overlay_app

app = typer.Typer(
    name="ccf",
    help="Code Context Fabric: code context fabric for AI coding agents.",
...
def _db_path(repo_root: Path) -> Path:
    # Directory name kept as ".repoweaver" (the project's former name) to avoid
    # invalidating already-indexed repos; revisit in a future release.
...
def check(
    ...
    if not db_path.exists():
        print("STALE (not indexed — run: ccf build)")
...
def init(
    ...
    """Initialise a new Code Context Fabric workspace (creates .repoweaver/ directory)."""
    repo_root = Path(repo).resolve()
...
def verify(
    ...
    from codecontextfabric.verify import run_verification
...
@app.command()
def version() -> None:
    """Print the installed Code Context Fabric version."""
    print(__version__)


def main() -> None:
    """Console entry point: `ccf` / `fabric` console scripts call this."""
    app()
