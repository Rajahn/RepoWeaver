# Code Context Fabric — AGENTS.md Injection Template

Copy the section below into the `AGENTS.md` (or `CLAUDE.md` / `CODEX.md`) at the
root of any repository indexed by Code Context Fabric. It gives your AI coding agent the
three rules it needs to use the `explore()` MCP tool correctly.

---

<!-- codecontextfabric:start -->
## Code Context Fabric Code-Context Rules
...
Never query an index that `ccf check` reports as stale — results will reflect
...
*This template is maintained by Code Context Fabric. Do not edit the three numbered sections
