...
def _built_fixture_repo(fixture: Path = _FIXTURE, name: str = "javademo"):
    with tempfile.TemporaryDirectory(prefix="codecontextfabric-verify-") as tmp:
...
def _run_m1_verification() -> VerifyResult:
    report: list[str] = [
        f"Code Context Fabric verify --level m1  (fixture: {_FIXTURE})"
    ]
    c = _Check(report)
...
def _query_cluster_rerank_checks(c: _Check) -> None:
    with tempfile.TemporaryDirectory(prefix="codecontextfabric-verify-query-") as tmp:
...
def _run_query_verification() -> VerifyResult:
    ...
    report: list[str] = ["Code Context Fabric verify --level query"]
    c = _Check(report)
...
def _run_benchmark_verification() -> VerifyResult:
    ...
    report: list[str] = ["Code Context Fabric verify --level benchmark"]
    c = _Check(report)
...
def _run_m2_verification() -> VerifyResult:
    ...
    report: list[str] = ["Code Context Fabric verify --level m2"]
    c = _Check(report)

    with tempfile.TemporaryDirectory(prefix="codecontextfabric-verify-m2-") as tmp:
...
def _perf_parallel_matches_serial_checks(repo_root: Path, c: _Check) -> None:
    import codecontextfabric.indexer as indexer_mod
    from codecontextfabric.benchmark.metrics import graph_signature
...
def _perf_incremental_sequence_checks(repo_root: Path, c: _Check) -> None:
    ...
    for step in range(n_steps):
        ...
        started = time.monotonic()
        with GraphStore(db_path) as store:
            Indexer(repo_root, store).build_incremental(changed={rel}, deleted=set())
            incremental_sig = graph_signature(store)
...
def _run_perf_verification() -> VerifyResult:
    ...
    report: list[str] = ["Code Context Fabric verify --level perf"]
    c = _Check(report)

    with tempfile.TemporaryDirectory(prefix="codecontextfabric-verify-perf-") as tmp:
...
def _m3_dispatch_and_overload_checks(store: GraphStore) -> tuple[dict, dict]:
    from codecontextfabric.graph.store import edge_id
...
def _run_m3_verification() -> VerifyResult:
    ...
    report: list[str] = ["Code Context Fabric verify --level m3"]
    c = _Check(report)
    ...
    with tempfile.TemporaryDirectory(prefix="codecontextfabric-verify-m3-") as tmp:
