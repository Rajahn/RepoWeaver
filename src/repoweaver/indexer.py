    ...
    def _try_fast_incremental(self, changed: set[str]) -> BuildStats | None:
        ...
        for row in self.store.conn.execute(
            ...
            old_identity[row["file"]].add(
                (row["kind"], row["qualified_name"], row["simple_name"], row["signature"])
            )
        ...
        changed_node_ids = {
            r.id for rows in node_rows_by_file.values() for r in rows
        }
        ...
        if new_super_edges != old_super_edges or new_super_unresolved != old_super_unresolved:
