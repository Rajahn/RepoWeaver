...
def run_verification(level: str, repo: Path) -> VerifyResult:
    if level != "m1":
        return VerifyResult(False, [f"Level '{level}' is not implemented in M1. Only 'm1' is supported."])
    ...
    with _built_fixture_repo() as repo_root:
        ...
        with GraphStore(db_path) as store:
            ...
            c.check("interface Greeter indexed", len(greeter_iface) == 1 and greeter_iface[0]["kind"] == "interface")
            level_enum = store.find_by_qualified_name("com.example.demo.Level")
            c.check("enum Level indexed", len(level_enum) == 1 and level_enum[0]["kind"] == "enum")
            ...
            fields = store.conn.execute("SELECT COUNT(*) FROM node WHERE kind = 'field'").fetchone()[0]
            c.check("at least one field indexed", fields >= 1, str(fields))

            report.append("-- edge resolution --")
            stats = store.stats()
            edge_types = stats["edge_types"]
            c.check("IMPLEMENTS edges resolved", edge_types.get("IMPLEMENTS", 0) >= 2, str(edge_types))
            c.check("EXTENDS edges resolved", edge_types.get("EXTENDS", 0) >= 1, str(edge_types))
            c.check("CALLS edges resolved", edge_types.get("CALLS", 0) >= 3, str(edge_types))
            c.check("IMPORTS edges resolved", edge_types.get("IMPORTS", 0) >= 1, str(edge_types))

            english_greeter = store.find_by_qualified_name("com.example.demo.EnglishGreeter")[0]
            implements_edges = [
                e for e in store.neighbors(english_greeter["id"], "out", 0.0) if e[1] == "IMPLEMENTS"
            ]
            c.check(
                "EnglishGreeter --IMPLEMENTS--> Greeter",
                any(e[0]["qualified_name"] == "com.example.demo.Greeter" for e in implements_edges),
            )

        report.append("-- retrieval (FTS + PPR) --")
        response = explore(query="Greeter", task="understand", repo=str(repo_root))
        c.check("explore() returns slices for 'Greeter'", len(response.get("slices", [])) > 0)
        hit_files = {s["file"] for s in response.get("slices", [])}
        c.check("retrieval finds Greeter.java", any("Greeter.java" in f for f in hit_files), str(hit_files))

        report.append("-- freshness --")
        response_fresh = explore(query="Greeter", task="locate", repo=str(repo_root))
        c.check("freshness reports 'ok' right after build", response_fresh["stats"]["freshness"] == "ok")
        ...
        c.check("freshness reports 'stale' after edit", response_stale["stats"]["freshness"] == "stale")
        ...
        c.check("blind_spots matches frozen contract string", response.get("blind_spots") == BLIND_SPOTS)
        for key in ("nodes_visited", "edges_traversed", "tokens_estimated", "freshness"):
        ...
        tight = explore(query="Greeter", task="understand", repo=str(repo_root), max_tokens=1)
        ...
        wide = explore(query="Greeter", task="understand", repo=str(repo_root), max_tokens=100000)
        c.check(
