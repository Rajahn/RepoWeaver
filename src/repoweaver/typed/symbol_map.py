"""Map SCIP symbol strings (maven/java scheme) onto RepoWeaver `qualified_name`s.
...
``(Ljava/lang/String;)``). RepoWeaver only ever needs Namespace/Type/Term/
...
def decode_jvm_param_types(disambiguator: str) -> list[str] | None:
    ...
    into RepoWeaver's simple, array/generics-erased type names (``int``,
    ...
    (RepoWeaver never guesses in that case)."""
    if disambiguator == "":
        return []
    out: list[str] = []
    pos = 0
...
class SymbolMapper:
    ...
    def _resolve_method(self, owner: str, member: Descriptor) -> MapResult:
        # RepoWeaver stores a constructor's `simple_name` as the owning
